  ...
  (let [text (or (format-viz-title title link) "Query results")]
    (case type
      :table (let [title-block {:type "section"
               ...
               blocks)
      :image (let [{:keys [file_id]} (or (slackbot.client/upload-image-file client content (str filename ".png"))
                                         (throw (ex-info "Slack image upload failed."
                                                         {:status-code 502
                                                          :type        :slack-image-upload-failed})))
                   blocks          [{:type "section"
                                     :text {:type "mrkdwn" :text text}}
                                    {:type       "image"
